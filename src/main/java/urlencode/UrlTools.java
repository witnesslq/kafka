...
import unicom.bitset.MobileShort;
		...
		if(message.contains("您的可用额度已不足"))//过滤低额提醒短信 
		{
			...
			if(message!=null)
			{
				...
				if(9<=messageArr.length)
				{
					...
					try {
						shortMobile=mobileShort.getShortMobile(mobile);
						//mobileNum=Long.parseLong(mobile);
						shortMobileNum=Integer.parseInt(shortMobile);//获取短号码
					...
					try {
						...
						isOpen=Diction.numBitSet.geMobileExist(shortMobileNum);//开通 
						isClose=Diction.numBitSet.geMobileNotExist(shortMobileNum);//关闭
					} catch (Exception e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
